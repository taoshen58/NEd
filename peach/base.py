...
def setup_prerequisite(args):
    # 1. output dir  # todo: how to resume training
    ...
    # 2. Initialize the accelerator. We will let the accelerator handle device placement for us in this example.
...
def setup_train_dataloader(args, train_dataset, accelerator, **kwargs):
    ...
    train_dataloader = DataLoader(
        train_dataset, shuffle=True, collate_fn=train_dataset.collate_fn,
...
def setup_eval_dataloader(args, eval_dataset, accelerator, use_accelerator=False, **kwargs):
    eval_dataloader = DataLoader(
        eval_dataset, collate_fn=eval_dataset.collate_fn, batch_size=args.per_device_eval_batch_size)
    if use_accelerator:
...
def save_model_with_default_name(args, accelerator, output_dir, model, tokenizer=None, args_to_save=None):
